...
class MainWindow(QWidget):
    ...
    def loadtickets(self):
        ...
        try:
            ...
            # JOIN to get AmountPaid from pays_for
            cursor.execute("""
            ...
            self.payments_table.setSortingEnabled(True)
            if self.payments_sort_info['col'] == -1:
                # No sort defined yet, apply default sorting on column 0 ascending
                self.payments_table.sortItems(0, Qt.SortOrder.AscendingOrder)
                self.payments_sort_info['col'] = 0
                self.payments_sort_info['order'] = Qt.SortOrder.AscendingOrder
                self.payments_table.horizontalHeader().setSortIndicator(0, Qt.SortOrder.AscendingOrder)
            else:
                # Apply existing sort info
                col = self.payments_sort_info['col']
                order = self.payments_sort_info['order']
                self.payments_table.sortItems(col, order)
                self.payments_table.horizontalHeader().setSortIndicator(col, order)
